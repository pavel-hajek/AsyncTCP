...
extern "C" {
    ...
    esp_err_t _tcp_output4ssl(tcp_pcb * pcb, void* client) {
        return _tcp_output(pcb, reinterpret_cast<AsyncClient *>(client)->_closed_slot);
    }

    esp_err_t _tcp_write4ssl(tcp_pcb * pcb, const char* data, size_t size, uint8_t apiflags, void* client) {
        return _tcp_write(pcb, reinterpret_cast<AsyncClient *>(client)->_closed_slot, data, size, apiflags);
    }
